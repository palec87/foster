defmodule FosterWeb.Components.Dashboard.Work do
  ...
  def update(_assigns, socket) do
    answers =
      ...
      |> Enum.group_by(fn answer -> get_in(answer.body, ["q8", "employment_status"]) end)
    ...
    data = answers |> Enum.map(fn [employment_status, count] -> %{"estado_emprego" => employment_status, "contagem" => count} end)
  ...
  def render(assigns) do
    ~H"""
    <div>
      <div id="work" phx-hook="DrawWork" style="margin-top: 20px; display: flex; justify-content: center;"></div>
    </div>
    """
  end
end
